...
export async function fetchAllArrivals(
  ...
  console.log(`[DBUS] Fetching arrivals for ${stops.length} stops on line ${lineNum}`);
  ...
  if (withEta.length > 0) {
    console.log(`[DBUS] Sample ETAs:`, withEta.slice(0, 5).map(r => `parada ${r.stopMarkerId}: ${r.etaMinutes}min (${r.direction})`));
  }
