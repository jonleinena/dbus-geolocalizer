...
function App() {
    ...
    const { lines, loading: linesLoading, error: linesError } = useLines();
    const { data: busData, loading: busesLoading, error: busesError } = useBuses(selectedLine);

    // Show the most relevant connection error
    const connectionError = busesError || linesError;
